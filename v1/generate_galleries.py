...
─────────────────────
Run from the same folder as VTT_Amazon_Catalog.html.
...
# ── config ────────────────────────────────────────────────────────────────────
THUMB_MAX      = (400, 400)   # max thumbnail dimensions (pixels)
THUMB_QUALITY  = 85           # JPEG quality for image thumbnails
LISTINGS_CSV   = '50581020586.csv'
INVENTORY_XLSX = 'item_numb_weight.xlsx'
...
# Camera CCM palette
INK     = (26,  24,  20)   # #1a1814
SURFACE = (237, 233, 225)  # #ede9e1
FAINT   = (138, 132, 128)  # #8a8480
ACCENT  = (200, 64,  26)   # #c8401a

# ── product name lookup (UPC → title) ────────────────────────────────────────

def load_product_names() -> dict:
    """Return {upc: title} by joining inventory Excel + listings CSV."""
    # Step 1: ASIN → UPC from inventory sheet
    asin_to_upc = {}
    inv_path = Path(INVENTORY_XLSX)
    if inv_path.exists():
        try:
            import pandas as pd
            inv = pd.read_excel(inv_path, sheet_name='Sheet1', header=None)
            for col in range(1, inv.shape[1]):
                try:
                    asin = str(inv.iloc[4, col]).strip() if pd.notna(inv.iloc[4, col]) else ''
                    upc  = str(inv.iloc[7, col]).strip() if pd.notna(inv.iloc[7, col]) else ''
                    if upc.endswith('.0'): upc = upc[:-2]
                    if re.match(r'^B0[A-Z0-9]{8}$', asin) and UPC_RE.match(upc):
                        asin_to_upc[asin] = upc
                except Exception:
                    pass
        except Exception as e:
            print(f'  [warn] could not read {INVENTORY_XLSX}: {e}')

    # Step 2: ASIN → title from listings CSV, mapped to UPC
    upc_to_title = {}
    csv_path = Path(LISTINGS_CSV)
    if csv_path.exists():
        try:
            import pandas as pd
            ...
            for _, r in df.iterrows():
                asin  = str(r['asin'])
                title = str(r['product-name']).strip()
                upc   = asin_to_upc.get(asin)
                if upc:
                    upc_to_title[upc] = title
        except Exception as e:
            print(f'  [warn] could not read {LISTINGS_CSV}: {e}')
...
<body>
  ...
  <main>
    <a class="back-link" href="../VTT_Amazon_Catalog.html">← Back to catalog</a>
    <div class="eyebrow">Artwork &amp; Press Images</div>
    <h1><em>Artwork</em> for<br>{short}</h1>
...
def main():
    ...
    if not upc_dirs:
        print('No UPC folders found. Create a folder named after a UPC number')
    ...
    for upc_dir in upc_dirs:
        ...
        # Gather source files, alphabetical by filename
        sources = sorted(
        ...
        for src in sources:
            ...
            entries.append((
                f'thumbs/{thumb_name}',  # thumb path — relative to gallery.html
                src.name,                # full-size path — same folder as gallery.html
                src.name,                # display label
            ))
    ...
    Path(GALLERIES_JS).write_text(js_content, encoding='utf-8')
